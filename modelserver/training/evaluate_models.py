...
def _measure(backend, rows: list[dict[str, str]]) -> tuple[float, float]:
    ...
    p95 = float(statistics.quantiles(latencies_ms, n=20)[18]) if len(latencies_ms) >= 20 else max(latencies_ms)
    return macro_f1, p95
...
def _run_candidate_c(rows: list[dict[str, str]], prompt: Path, hosted_id: str, price_per_1k_tokens: float, avg_tokens_per_call: float) -> CandidateResult:
    ...
    return CandidateResult(
        ...
        notes=f"Hosted LLM {hosted_id}. Cost = {price_per_1k_tokens}/1k tokens × {avg_tokens_per_call} avg tokens/call.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--artifact-onnx", type=Path, default=REPO_ROOT / "modelserver" / "artifacts" / "classifier.onnx")
    parser.add_argument("--artifact-joblib", type=Path, default=REPO_ROOT / "modelserver" / "artifacts" / "classifier.joblib")
    parser.add_argument("--prompt-c", type=Path, default=REPO_ROOT / "modelserver" / "artifacts" / "classifier_prompt.txt")
